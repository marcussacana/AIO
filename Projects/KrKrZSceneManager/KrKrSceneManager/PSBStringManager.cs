...
namespace KrKrSceneManager {
    public class PSBStrMan {  
        ...
        public string[] Import() {
            PackageStatus Status = GetPackageStatus(Script);
            switch (Status) {
                case PackageStatus.Invalid:
                    throw new Exception("Invalid Package");
                case PackageStatus.MDF:
        ...
        public byte[] TryRecovery()
        {
            ...
            var Status = GetPackageStatus(Script);
            if (Status == PackageStatus.Invalid)
                throw new Exception("Invalid Package");

            bool MDF = Status == PackageStatus.MDF;
            if (MDF)
                Script = ExtractMDF(Script);
        ...
        public static PackageStatus GetPackageStatus(byte[] Package) {
            if (ReadOffset(Package, 0, 3) == 0x66646D)
                return PackageStatus.MDF;
            if (ReadOffset(Package, 0, 3) == 0x425350)
