...
namespace KrKrSceneManager {
    ...
    public class PSBResManager {
        ...
        public FileEntry[] Import(byte[] script) {
            PSBStrMan.PackageStatus Status = PSBStrMan.GetPackageStatus(script);
            if (Status == PSBStrMan.PackageStatus.MDF)
                script = PSBStrMan.ExtractMDF(script);
            Status = PSBStrMan.GetPackageStatus(script);
            if (Status != PSBStrMan.PackageStatus.PSB)
                throw new Exception("Bad File Format");

            Package = script;
            StartPos = PSBStrMan.ReadOffset(Package, 0x20, 4);
            OffsetPos = PSBStrMan.ReadOffset(Package, 0x18, 4);
            int[] tmp = GetOffsetInfo(Package, OffsetPos);
            OffsetSize = tmp[0];
            OffsetTablePos = tmp[1];
            ResSizePos = PSBStrMan.ReadOffset(Package, 0x1C, 4);
            tmp = GetOffsetInfo(Package, ResSizePos);
            ResSizeOffSize = tmp[0];
            ResSizeOffTablePos = tmp[1];
            int[] Offsets = GetValues(Package, OffsetPos);
            int[] Sizes = GetValues(Package, ResSizePos);
            ...
            for (int i = 0; i < EntryCount; i++) {
                ...
                for (int ind = Offsets[i]; ind < EndPos; ind++)
                    data[ind - Offsets[i]] = Package[ind + StartPos];
        ...
        public byte[] Export(FileEntry[] Resources) {
            ...
            TotalSize = 0;
            byte[] MainData = CutAt(Package, StartPos);
            for (int i = 0; i < Resources.Length; i++) {
            ...
            byte[] ResultPackage = new byte[MainData.Length + ResTable.Length];
            MainData.CopyTo(ResultPackage, 0);
            ResTable.CopyTo(ResultPackage, MainData.Length);
            return CompressPackage ? PSBStrMan.CompressMDF(ResultPackage) : ResultPackage;
        }
